...
// Create Region String and Table HTML
foreach($trips as $key => $trip){
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        $regionString .= $name . ' ' . $type . '/';
        $regionHTML .= "<tr><td>".$type."</td><td>".$name."</td></tr>";
    }

    $strings[] = substr($regionString, 0, -1);
...
foreach($trips as $key => $trip){
    $qsent = $trip['stoves'];
    $delivery = $qsent . ' stoves to ' . $strings[$key];
    $pDelivery = $trip['posters'] . ' posters to ' . $strings[$key];
