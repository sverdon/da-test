   ...
   $oldestFile = $files[0]; // the latest modified file should be the first. 
    ...
    while( ($row = fgetcsv($file)) != FALSE )
    {
                ...
                if(array_key_exists($i, $assocArray)) //field is one of the fields that needs to get added to table
                {
                    ...
                    //if(RegularForm == 1 || QRForm == 1){
                        ...
                        if($i == $sysIDindex)
                        {
                            ...
                            /*  //code to update 'HHSize_Under5' in the table
                                if($formID = '41628408'){
                                    while($fieldName_DBtbl != 'HHSize_Under5' && $i < $count)
                                    {
                                        if(array_key_exists($i, $assocArray)) //field is one of the fields that needs to get added to table
                                        {
                                            $fieldName_DBtbl = $assocArray[$i]['FieldName_DBtbl'];
                                            if($fieldName_DBtbl == 'HHSize_Under5')
                                            {
                                                $sql = "UPDATE fd_HHVs SET HHSize_Under5" . " = '" .$row[$i]."' WHERE SystemID = $sysID;";
                                                $result = mysqli_query($conn_da, $sql);
                                                if(!$result){
                                                    echo "Failed to update HHSize_Under5" .PHP_EOL;
                                                    echo $sql . PHP_EOL;
                    ...
                    */
                    ...
                    if($i != $sysIDindex && $i != $dateValueIndex && $timesFound <= 1 && $i != $QRcodeIndex)
                    {
                        if($tableName != 'tble_LogTransOutNin') //specific to QR forms
                        {
                            ...
                            if( (strpos($fieldName_DBtbl,'OLC_Other') !== FALSE) && ($tableName == 'fd_HHVs') )
                            {
        ...
        if($flag == 1) //means row got split
        {
            ...
            if(RegularForm == 1 && ($columns != '' && $values != '') ){
                //4) Insert data into table
